TestPrepare = function (options) {
    ...
    this.fs = require('fs');
    this.test_database = options.test_database || `my_test_prepare_database_${new Date().getTime()}`;
    this.mongo_host = options.mongo_host;
    this.mongo_user = options.mongo_user || '';
    this.mongo_password = options.mongo_password ? `:${options.mongo_password}@` : '';
    this.mongo_uri = `mongodb://${this.mongo_user}${this.mongo_password}${this.mongo_host}/${this.test_database}`;
...
TestPrepare.prototype._connect = function () {
    ...
    return new Promise((resolve, reject) => {
        if (base.mongoose.connection.readyState == 0) {
            base.mongo = ((() => {
                base.mongoose.connect(base.mongo_uri, { useMongoClient: true }, function (err) {
                if (err)
                    reject(err);
                else {
                ...
                return base.mongoose.connection;
            })());
        }
        else {
...
TestPrepare.prototype._importFixturesCustomSchema = function (fixtures) {
    var base = this;
    var promises = [];
    ...
    // import all fixtures
    for (var fixture of fixtures)
        promises.push(base._importFixture(fixture.data, null, fixture.model))

    return Promise.all(promises);
};

// Imports one fixture, and apply middleware if is defined.
TestPrepare.prototype._importFixture = function (fixture, middleware, schemaDefinition) {
    var base = this;
    ...
    return new Promise((resolve, reject) => {
        base.fs.readFile(path, 'utf8', function (err, data) {
            if (!err) {
                ...
                let schema = base.mongo.models[item.model];

                if (!schema) {
                    const mongoSchema = schemaDefinition ? schemaDefinition : {};
                    schema = base.mongo.model(item.model, base.mongoose.Schema(mongoSchema, { strict: false, collection: item.model }));
                }

                schema
                    .insertMany(item.fixtures, function (err, result) {
                        if (!err)
...
TestPrepare.prototype._clear = function () {
    ...
    return new Promise((resolve, reject) => {
        if (base._isTestPrepareDatabase()) {
            base.mongo.base.connection.dropDatabase(function () {
...
// Starts connections, clean database and import fixtures
TestPrepare.prototype.startCustomSchema = function (fixtures) {
    ...
    return new Promise(async (resolve, reject) => {
        const conn = await this._connect()
        this._clear()
        .then(() => this._importFixturesCustomSchema(fixtures))
        .then(() => resolve(conn));
    })
};
...
TestPrepare.prototype._isTestPrepareDatabase = function () {
    var isTestPrepareDatabase = this.mongo.base.connection.name == this.test_database;
