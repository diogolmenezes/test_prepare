    ...
    return new Promise((resolve, reject) => {        
        if (base.mongoose.connection.readyState == 0) {
            base.mongo = base.mongoose.connect(base.mongo_uri, { user: base.mongo_user, pass: base.mongo_password }, function (err) {
                ...
                {
                    if(base.verbose)
                        console.log(`Test Prepare => Mongo is connected on (${base.mongo_uri})!`);
        ...
        base.fs.readFile(path, 'utf8', function (err, data) {
            if (!err) {
                ...
                base.mongo.model(item.model).insertMany(item.fixtures, function (err, result) {
                     if (!err && base.verbose)
                        console.log(`Test Prepare => Fixture [${fixture}] was loaded.`);
...
TestPrepare.prototype._clear = function () {
    var base = this;
    return new Promise((resolve, reject) => {
        var isTestPrepareDatabase = base.mongo.connection.name.indexOf('test_prepare') != -1;
        if (isTestPrepareDatabase)
        {
            base.mongo.connection.dropDatabase(function() {
                if(base.verbose)
                    console.log(`Test Prepare => The database [${base.test_database}] is now clear.`);
...
TestPrepare.prototype.start = function (fixtures) {
    if(this.verbose)
        console.log('Test Prepare => The test will be prepared now.');
